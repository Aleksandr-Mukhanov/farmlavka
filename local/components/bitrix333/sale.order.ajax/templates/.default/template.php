...
$this->addExternalJs($templateFolder.'/jquery.suggestions.min.js');
?>
	<NOSCRIPT>
...
else
{
	...
	<form action="<?=POST_FORM_ACTION_URI?>" method="POST" name="ORDER_FORM" id="bx-soa-order-form" enctype="multipart/form-data">
		...
		<div id="bx-soa-order" class="checkout__body" style="opacity: 0">
			<!--	MAIN BLOCK	-->
			<div class="checkout__left bx-soa">
				...
				<? if ($arParams['DELIVERY_TO_PAYSYSTEM'] === 'p2d'): ?>
					...
					<!--	DELIVERY BLOCK	-->
					<div id="bx-soa-delivery" data-visited="false" class="bx-soa-section bx-active checkout__ch-order" <?=($hideDelivery ? 'style="display:none"' : '')?>>
						<div class="bx-soa-section-title-container">
							<h2 class="bx-soa-section-title col-sm-9">
			...
			<!--	SIDEBAR BLOCK	-->
			<div id="bx-soa-total" class="b-main__right bx-soa-sidebar">
...
$(".checkout__ch-order").each(function() {
var titleblock = $(this).find('h2').text();
$(this).find('.bx-soa-section-title-container').remove();
$(this).prepend('<h3 class="footer__question__title">'+titleblock+'</h3>');
});
rebuildcheckout();
$(document).on('click',"#bx-soa-delivery .bx-soa-pp-company",function(){
setTimeout(function() {
rebuildcheckout();
}, 500);
});
...
$(document).on('click',"#dibSpan",function(){
$(this).toggleClass('active').parent('#DopInfoBtn').next().slideToggle();
});


function rebuildcheckout() {
$('#bx-soa-delivery .bx-soa-pp').append('<p class="ch-delivery__text">Выберите адрес из списка или добавьте новый:</p><li class="availability__item"><input class="availability__item__input" type="radio" id="contactChoice1" name="availability" value="yes"><label class="availability__item__label" for="contactChoice1">Добавить новый адрес</label></li><div id="delivery-fields" class="bx-soa-customer"></div><div id="DopInfoBtn"><span id="dibSpan" class="ch-delivery__text-bottom">Дополнительная инфомация по адресу</span></div><div id="DopInfoSlide" style="display: none;"><div id="DopInfoContent"></div></div>');
$("#delivery-fields").append($('#bx-soa-properties .bx-soa-customer-field[data-property-id-row="10"]').detach());
$("#delivery-fields").append($('#bx-soa-properties .bx-soa-customer-field[data-property-id-row="6"]').detach());
$("#delivery-fields").append($('#bx-soa-properties .bx-soa-customer-field[data-property-id-row="7"]').detach());
$("#delivery-fields").append($('#bx-soa-properties .bx-soa-customer-field[data-property-id-row="8"]').detach());
$("#delivery-fields").append($('#bx-soa-properties .bx-soa-customer-field[data-property-id-row="9"]').detach());
$("#delivery-fields").append($('#bx-soa-properties .bx-soa-customer-field[data-property-id-row="11"]').detach());
$("#DopInfoContent").append($('#bx-soa-properties .bx-soa-customer-field[data-property-id-row="12"]').detach());
$("#DopInfoContent").append($('#bx-soa-properties .row>.col-sm-12:last-child>.bx-soa-customer-field:last-child').attr('id', 'customer-comment').detach());
var labletext = $('#customer-comment label').text();
var labletext = labletext.replace(":", "");
$('#orderDescription').attr('placeholder', labletext);
$('#soa-property-10').before('<p id="message"></p>');
$('body').addClass('good');
}

$("#soa-property-10").suggestions({
  // Замените на свой API-ключ
  token: "2a4fe6b3f5ec8531916f19c2daf18548ee4ada0e",
  type: "ADDRESS",
  onSelect: validate
});
