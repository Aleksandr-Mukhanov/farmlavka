...
<ul class="reviews-right__body" id="reviews-list" data-primary-color="<?=$arParams['PRIMARY_COLOR']?>">
<?
$frame = $this->createFrame()->begin();
global $APPLICATION;
global $USER;
?>
	<!-- <li class="review__li" data-items-count="<?=$arResult["REVIEWS_FILTER_CNT"]?>" data-date-format="<?=$arParams['DATE_FORMAT']?>"> -->
		<?if(isset($arResult['REVIEWS']) && is_array($arResult['REVIEWS'])){?>
			<?foreach($arResult['REVIEWS'] as $Review){?>
				<li class="review__li" data-id="<?=$Review['ID']?>" data-site-dir="<?=SITE_DIR?>" id="review-<?=$Review['ID']?>" itemscope itemtype="http://schema.org/Review">
					<div class="review__top">
						<div class="rb__flex">
							<p class="review__title" itemprop="author" itemscope itemtype="http://schema.org/Person">
								<span itemprop="name"><?=$Review['NAME']?></span>
								<span class="review__span" itemprop="datePublished"><?=$Review['DATE_CREATION'];?></span>
							</p>
